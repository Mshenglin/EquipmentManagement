...
CREATE TABLE `operational_logs` (
                                    ...
                                    `operational_id` TINYINT(2) UNSIGNED NOT NULL DEFAULT '0' COMMENT '操作类型，1增加，2修改,3，删除',
                                    `remark` TEXT  DEFAULT NULL COMMENT '备注',
                                    PRIMARY KEY (`id`)
)  ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='操作日志表';
DROP TABLE IF EXISTS `operational_type_logs`;
-- 实验室器材类型操作日志表
CREATE TABLE `operational_type_logs` (
                                         `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'id',
                                         `username` varchar (20) NOT NULL  COMMENT '用户名称',
                                         `equipment_type_id` int NOT NULL COMMENT '实验器材类型id',
                                         `create_time` BIGINT(20)  NOT NULL COMMENT '操作时间',
                                         `operational_id` TINYINT(2) UNSIGNED NOT NULL DEFAULT '0' COMMENT '操作类型，1增加，2删除,3修改',
